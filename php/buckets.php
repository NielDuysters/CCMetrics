<?php
    ...
    if (isset($_GET['save'])) {
        ...
        $sql = "UPDATE metric_data_opunits SET ".$bucket."=? WHERE ID=?";
    ...
    $sql = "select mdo.id, def.definition, target, expr, bucket1_bottom, bucket1_upper, bucket2_bottom, bucket2_upper, bucket3_bottom, bucket3_upper, bucket3_bottom, bucket3_upper, bucket4_bottom, bucket4_upper, bucket5_bottom, bucket5_upper FROM metric_data_opunits mdo INNER JOIN definitiondetails dd ON dd.id=mdo.definitiondetails INNER JOIN definitions def on def.id=dd.definition_id" . $do_where . implode(" AND ", $where) . " AND (dd.competence_id=? OR dd.competence_id=?)";
    ...
    while ($stmt->fetch()) {
        ...
        <tr data-bucket-id='".$id."'>
            ...
            <td class='editable' contenteditable='true' data-bucket-nr='2_bottom'>".$bucket2_bottom."</td>
            <td class='editable' contenteditable='true' data-bucket-nr='2_upper'>".$bucket2_upper."</td>
            ...
            <td class='editable' contenteditable='true' data-bucket-nr='4_bottom'>".$bucket4_bottom."</td>
            <td class='editable' contenteditable='true' data-bucket-nr='4_upper'>".$bucket4_upper."</td>
